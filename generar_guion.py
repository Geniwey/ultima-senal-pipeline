...
- TÍTULO CON BRECHA DE CURIOSIDAD: nunca un título descriptivo tipo "Investigación Aérea: El Vuelo 603". Usa un título que genere una pregunta en la mente del espectador sin responderla — ej. "El avión que cayó al mar por un trozo de cinta" o "Volaron a ciegas hacia su destino (y nadie lo supo hasta después)". Nombra la causa curiosa sin explicarla del todo.
- SOLO UNA LLAMADA A SUSCRIBIRSE EN TODO EL GUION, exactamente una vez, colocada alrededor del 75% del guion (no antes) — justo después de resolver la causa raíz del accidente, aprovechando que ya has entregado el valor prometido. Nunca al principio ni a mitad de la tensión.
...
- CIERRE CON GANCHO A OTRO VÍDEO, no despedida pasiva: la última escena no debe ser solo "gracias por ver" — debe terminar con una frase que invite a quedarse en el canal, tipo "Si esto te ha dejado con la boca abierta, espera a conocer el caso que viene" (sin inventar un caso concreto, solo la promesa).
...
Devuelve SOLO un JSON válido, sin texto adicional, con esta forma exacta:
{
  ...
  "comentario_fijado": "una pregunta polarizante/de debate relacionada con el caso, para fijar en los comentarios y generar interacción (ej. '¿Crees que fue un fallo humano o de protocolo? Os leo')",
  "escenas": [
    {
